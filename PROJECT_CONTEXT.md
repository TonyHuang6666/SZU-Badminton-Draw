...
当前主分支：`main`  
当前接续提交：`678fc12 Harden visual exports and macOS packaging`
...
- 未填胜方、比分与胜方方向不一致、弃权无比分等作为弹窗提醒，可手动确认继续。
- 未完成比赛可顺延到下一比赛日记录表。
...
当前已经支持一次导入多张历史记录表，但每次都要用户重新选择历史文件。下一步应生成并维护一个赛事进度文件，例如：
...
- `累计结果`：所有已完成场次、胜方、负方、比分、用时、来源记录表。
- `原始赛程索引`：完整赛程每场的 MatchName、日期、时间、场地、双方来源。
- `赛程参数`：比赛日、时间、场地、单场时长、每日最多场次、分界线设置。
- `导入日志`：每次导入的文件、时间、读取场次、警告、冲突。
...
- `BadmintonDraw.Excel`：进度文件读写。
- `BadmintonDraw.Workflows`：导入记录表并更新进度文件的业务流程。
- `BadmintonDraw.App` / `BadmintonDraw.Desktop`：按钮、文件选择、提醒弹窗。
...
1. 做 `赛事进度文件 / 累计结果持久化`。
2. 让 Windows 和 macOS 同时接入这个进度文件流程。
3. 做 `导入结果后导出下一日完整材料包`。
4. 做基础版 `单场比赛计分表`。
5. 做跨项目冲突报告。
6. 做高级赛程约束。
...
1. 新增进度文件 writer/reader。
2. 能把 `MatchRecordImportResult` 写入 `累计结果`。
3. 能从进度文件读取历史累计结果。
4. 导入新记录表时检测和历史结果冲突。
5. Windows 和 macOS 都提供入口。
6. 补测试：写入、读取、重复一致、重复冲突、导出下一日记录表。
