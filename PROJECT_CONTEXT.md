...
已完成并同步接入 Windows WPF 与 macOS Avalonia：

```text
生成完整赛程
-> 导出首日材料包
-> 包含首日记录表、赛程安排表 xlsx/pdf、带时间场地对阵表 xlsx/pdf、单场计分表 pdf（每场一页）

导入记录表/进度文件
-> 更新累计结果
-> 选择材料包文件夹
-> 导出下一比赛日记录表
-> 导出下一比赛日赛程安排表 xlsx/pdf
-> 导出下一比赛日带时间场地对阵表 xlsx/pdf
-> 导出下一比赛日单场比赛计分表 pdf（每场一页）
-> 团体项目额外导出团体赛记分表 xlsx
```

单场比赛计分表只生成 PDF，按新版 `羽毛球比赛记分表.xlsx` 模板生成，每场比赛一页横版，页面整体偏右以给 A4 板夹留左侧夹持空间。表内序号来自赛程记录表第一列，方便裁判长回收后快速定位、对照和统计赛果；未决出具体姓名的占位场次不写入姓名区，留给裁判员现场手填。

团体赛记分表已从旧 Word 模板路线改为 Excel，保留旧模板的标题、双方队名、阶段、组别、日期、时间、场号、比赛结果、获胜队和裁判长签名，并增加分场记录。

格式取舍：

- 抽签结果和完整赛程表保留 Excel/JPG/PNG/A4 PDF/全部导出，面向留档、公示和临时分享。
- 现场材料包固定生成 `.xlsx` 和 `.pdf`，其中记录表只生成 `.xlsx`，单场计分表只生成 `.pdf`，团体记分表只生成 `.xlsx`，避免现场文件过多。
...
1. 允许导入或选择多个项目的赛程/赛事存档。
2. 按选手姓名抽取同一人跨项目参赛记录。
3. 检查同一时间段冲突和间隔过短风险。
4. 先导出报告供裁判长人工调整，不急于自动重排。
5. Windows 和 macOS 同时接入，并补共享 workflow 测试。
