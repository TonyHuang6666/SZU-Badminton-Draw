...
第一阶段 `BadmintonDraw.Desktop` 已接入：
...
1. 不改动抽签核心算法来适配 UI。
2. 新 UI 只调用 `BadmintonDraw.Core` 和 `BadmintonDraw.Excel` 的公开接口。
...
1. 把 WPF `MainWindow.xaml.cs` 中的业务流程抽成共享服务或 ViewModel。
2. 迁移赛程编排基础页。
3. 迁移赛程记录表导入、提醒确认、未决顺延工作流。
4. 验证 ClosedXML 和 SkiaSharp 在 macOS 上的中文字体、图片和 PDF 输出。
5. 增加 macOS 打包、签名、公证脚本。
