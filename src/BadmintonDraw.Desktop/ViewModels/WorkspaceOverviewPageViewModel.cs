...
public sealed class WorkspaceOverviewPageViewModel : WorkspacePageViewModel
{
    ...
    private TournamentWorkspace? editorBaseline;
    private bool configurationConflict;
    ...
    public WorkspaceOverviewPageViewModel(AppShellViewModel shell, WorkspaceSession session) : base(session)
    {
        ...
        }, () => CanEditFields && !configurationConflict, shell.ReportError);
    ...
    {
        editorBaseline = Session.Workspace;
    ...
    {
        // A newer read snapshot is not consent to submit retained inputs against a changed configuration.
        // Keep the conflict latched until the user cancels/reopens the editor, even if a later update changes it back.
        if (IsEditing && editorBaseline is not null && !SameConfiguration(editorBaseline, next.Workspace))
    ...
    private static bool SameConfiguration(TournamentWorkspace baseline, TournamentWorkspace current) =>
        ...
        baseline.Projects.OrderBy(p => p.SortOrder).Select(p => (p.Id, p.Discipline, p.DisplayName, p.CompetitionMode, p.SortOrder))
            .SequenceEqual(current.Projects.OrderBy(p => p.SortOrder).Select(p => (p.Id, p.Discipline, p.DisplayName, p.CompetitionMode, p.SortOrder)));
